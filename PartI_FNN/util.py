...
def func_confusion_matrix(y_test, y_pred):
    ...
    y_test = np.array(y_test)  # y_test input
    y_pred = np.array(y_pred)  # y_pred input

    unique_values = set(y_pred)
    sorted(unique_values)
    num_classes = len(unique_values)
    # change to array so can use indexes
...
def _perform1point1(confidence_threshold):
    ...
    y_pred_conf = [0.95, 0.83, 0.78, 0.53,
                   0.9, 0.2, 0.6, 0.8, 0.7, 0.5, 0.9, 0.2]
